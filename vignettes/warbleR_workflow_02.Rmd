---
title: <center><font size="7"><b>warbleR&#58; Visual inspection and signal classification</b></font></center>
author: 
...
vignette: >
  ...
  %\VignetteIndexEntry{warbleR: 02 - Visual inspection and signal classification}
...
Overlapping selections can be common after using `autodetec` to select signals, but can sometimes arise after selecting signals using other functions or software. The function below helps you detect overlapping signals in your selection table, and has arguments that you can play around with for overlap detection, renaming or deleting overlapping selections.
```{r, echo=TRUE, eval=FALSE}
...
Check out the image file in your working directory. These will look very similar to the `lspec` images produced in vignette 1, but with red dotted lines indicating where the selected signals start and end. 
...
This section on `catalog` is taken from [Marcelo Araya-Salas's bioacoustics GitHub blog](https://marce10.github.io/2017/03/17/Creating_song_catalogs.html) with slight modifications. 
...
The next vignette will cover the third phase of the _warbleR_ workflow, which includes methods to perform acoustic mesaurements as a batch process, an example of how to use these measurements for an analysis of geographic variation, coordinated singing analysis and a new function to simulate songs.
